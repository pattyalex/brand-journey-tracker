...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await stripeRequest('/v1/billing_portal/sessions', 'POST', {
      customer: customerId,
      return_url: returnUrl,
    }, key);

    if (result.status !== 200) {
      return res.status(500).json({ error: result.body.error?.message || 'Failed to create portal session' });
    }
