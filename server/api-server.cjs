...
// MegAI Chat endpoint - for conversational AI in script editor
app.post('/api/megai-chat', async (req, res) => {
  try {
    const { systemPrompt, messages, maxTokens = 800 } = req.body;
    const apiKey = process.env.VITE_ANTHROPIC_API_KEY;
    ...
    console.log('MegAI chat request:', messages.length, 'messages');
    ...
    const data = await response.json();
    console.log('MegAI response generated');

    res.json({ content: data.content[0].text });
  } catch (error) {
    console.error('Error in MegAI chat:', error);
    res.status(503).json({ error: 'AI service is temporarily unavailable. Please try again.' });
  }
});

// Generate subcategories endpoint - for Production page pillar expansion
app.post('/api/generate-subcategories', async (req, res) => {
  try {
    ...
    console.log('Generating subcategories for pillar:', pillarName);
...
Return JSON array only.`
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        console.log('Generated', subcategories.length, 'subcategories');
...
// Generate content ideas endpoint - for Production page
app.post('/api/generate-content-ideas', async (req, res) => {
  try {
    ...
    if (!pillarName || !subCategory) {
      return res.status(400).json({ error: 'Pillar name and subcategory are required' });
    }
...
// Generate angles endpoint - for Production page idea expansion
app.post('/api/generate-angles', async (req, res) => {
  try {
    const { ideaText, count = 10 } = req.body;
    const apiKey = process.env.VITE_ANTHROPIC_API_KEY;
    ...
    console.log('Generating angles for idea:', ideaText.substring(0, 50) + '...');
