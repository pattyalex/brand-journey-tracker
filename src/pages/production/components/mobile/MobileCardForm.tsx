...
interface MobileCardFormProps {
  ...
  onDelete: () => void;
  onSaveCard: (updatedCard: ProductionCard) => void;
}

const MobileCardForm: React.FC<MobileCardFormProps> = ({
  card,
  columns,
  onClose,
  onOpenStoryboard,
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-auto" style={{ background: '#F8F6F4' }}>
      ...
      <div className="relative z-10 p-5 space-y-5 pb-32">
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #612a4f 0%, #8B7082 100%)' }}
              >
                ...
                <>
                  ...
                  <div className="h-2 rounded-full bg-white/20 mb-5 overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(filmedScenes.size / card.storyboard.length) * 100}%` }}
                    />
                  </div>
            ...
            {(card.locationText || card.outfitText || card.propsText || card.filmingNotes) && (
              ...
              >
                ...
                {expandedSections.has('filmPrep') && (
                  <div className="px-5 pb-5 space-y-1">
                    {card.locationText && (
                      <div className="flex items-start gap-3 py-3 px-3 rounded-xl" style={{ background: 'rgba(139, 112, 130, 0.05)' }}>
                        <MapPin className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: '#8B7082' }} />
                        <span className="text-sm" style={{ color: '#1a1523' }}>{card.locationText}</span>
                      </div>
                    )}
                    {card.outfitText && (
                      <div className="flex items-start gap-3 py-3 px-3 rounded-xl" style={{ background: 'rgba(139, 112, 130, 0.08)' }}>
                        <Shirt className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: '#8B7082' }} />
                        <span className="text-sm" style={{ color: '#1a1523' }}>{card.outfitText}</span>
                      </div>
                    )}
                    {card.propsText && (
                      <div className="flex items-start gap-3 py-3 px-3 rounded-xl" style={{ background: 'rgba(139, 112, 130, 0.11)' }}>
                        <Boxes className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: '#8B7082' }} />
                        <span className="text-sm" style={{ color: '#1a1523' }}>{card.propsText}</span>
                      </div>
                    )}
                    {card.filmingNotes && (
                      <div className="flex items-start gap-3 py-3 px-3 rounded-xl" style={{ background: 'rgba(139, 112, 130, 0.14)' }}>
                        <NotebookPen className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: '#8B7082' }} />
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-lg font-semibold" style={{ color: '#612a4f', fontFamily: "'Playfair Display', serif" }}>
              ...
              <div className="grid grid-cols-7 gap-1 mb-1">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, i) => (
                  <div key={i} className="h-8 flex items-center justify-center text-[11px] font-semibold uppercase tracking-wide" style={{ color: '#8B7082' }}>
                    {day}
                  </div>
                ))}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {hasScheduledContent && day.isCurrentMonth && (
                        <div className="absolute bottom-1.5 w-1.5 h-1.5 rounded-full" style={{ background: day.isSelected ? 'white' : '#10b981' }} />
                      )}
                    </button>
                  );
                })}
              ...
              {scheduledDate && scheduledContent[scheduledDate] && scheduledContent[scheduledDate].length > 0 && (
                <div className="mt-4 p-3 rounded-2xl" style={{ background: 'rgba(16, 185, 129, 0.08)' }}>
                  <p className="text-xs font-medium mb-2" style={{ color: '#059669' }}>Also scheduled for this day:</p>
                  <div className="space-y-2">
                    {scheduledContent[scheduledDate].map((item) => {
                      ...
                      return (
                        <div key={item.id} className="flex items-center gap-2 text-sm" style={{ color: '#1a1523' }}>
                          <span className="text-xs px-2 py-0.5 rounded-md font-medium" style={{ background: 'rgba(16, 185, 129, 0.15)', color: '#059669' }}>
                            {hour12}{ampm}
                          </span>
              ...
              {scheduledDate && (
                <button
                  onClick={() => { setScheduledDate(''); setScheduledStartTime('09:00'); }}
                  className="w-full py-2.5 mt-4 rounded-xl text-sm font-medium transition-all active:scale-[0.98]"
                  style={{ background: 'rgba(139, 112, 130, 0.08)', color: '#8B7082' }}
                >
                  Clear date
                </button>
              )}
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#8B7082' }}>Post Time</label>
                <span className="text-sm font-semibold px-3 py-1 rounded-lg" style={{ background: 'rgba(97, 42, 79, 0.1)', color: '#612a4f' }}>
                  {(() => {
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(97, 42, 79, 0.1)' }}>
                    <FileText className="w-4 h-4" style={{ color: '#612a4f' }} />
                  </div>
                ...
                <ChevronDown
                  className="w-5 h-5 transition-transform duration-300"
                  style={{ color: '#8B7082', transform: expandedSections.has('contentSummary') ? 'rotate(180deg)' : 'rotate(0deg)' }}
                />
              </button>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {card.platforms.map((platform) => (
                          <span key={platform} className="px-3 py-1 rounded-full text-xs" style={{ background: 'rgba(97, 42, 79, 0.1)', color: '#612a4f' }}>
                            {platform}
                          </span>
                        ))}
                      </div>
        ...
        {showScheduleConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'rgba(16, 185, 129, 0.4)', backdropFilter: 'blur(4px)' }}>
            <div className="w-full max-w-sm rounded-3xl p-6" style={{ background: 'white', boxShadow: '0 25px 60px rgba(16, 185, 129, 0.25)' }}>
              ...
              <h3 className="text-xl font-semibold mb-2 text-center" style={{ color: '#1a1523', fontFamily: "'Playfair Display', serif" }}>Schedule Content?</h3>
              <p className="text-sm mb-2 text-center" style={{ color: '#6b6478' }}>This content will be scheduled for:</p>
              <p className="text-base font-semibold mb-6 text-center" style={{ color: '#059669' }}>
                {scheduledDate && new Date(scheduledDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  style={{ background: 'rgba(139, 112, 130, 0.1)', color: '#612a4f' }}
                >
                  Cancel
                </button>
                <button
        ...
        {showMarkPosted && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'rgba(97, 42, 79, 0.4)', backdropFilter: 'blur(4px)' }}>
            <div className="w-full max-w-sm rounded-3xl p-6" style={{ background: 'white', boxShadow: '0 25px 60px rgba(97, 42, 79, 0.25)' }}>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)' }}>
                <Send className="w-7 h-7 text-white" />
              </div>
              <h3 className="text-xl font-semibold mb-2 text-center" style={{ color: '#1a1523', fontFamily: "'Playfair Display', serif" }}>Mark as Posted?</h3>
              <p className="text-sm mb-6 text-center" style={{ color: '#6b6478' }}>This will move the content to your Posted archive.</p>
              <div className="flex gap-3">
                <button onClick={() => setShowMarkPosted(false)} className="flex-1 py-3.5 rounded-xl font-medium transition-all" style={{ background: 'rgba(139, 112, 130, 0.1)', color: '#612a4f' }}>Cancel</button>
                <button
                  onClick={() => {
                    onMove('posted');
                    setShowMarkPosted(false);
                    toast.success('Card moved to archive', { description: 'View your archive on desktop' });
                    onClose();
                  }}
                  className="flex-1 py-3.5 rounded-xl font-medium transition-all"
                  style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white' }}
        ...
        {card.columnId === 'posted' && (
          ...
          >
            <label className="block text-xs font-semibold mb-3 uppercase tracking-wider" style={{ color: '#8B7082' }}>Notes</label>
            <textarea
        ...
        >
          <label className="block text-xs font-semibold mb-3 uppercase tracking-wider" style={{ color: '#8B7082' }}>Move to</label>
          <button
          ...
          >
            ...
            <ChevronDown
              className="w-5 h-5 transition-transform duration-300"
              style={{ color: '#612a4f', transform: showMoveMenu ? 'rotate(180deg)' : 'rotate(0deg)' }}
            />
          </button>
            ...
            <div className="mt-4 space-y-2">
              {columns.map((col) => (
                <button
                  ...
                  style={{
                    background: col.id === card.columnId ? 'linear-gradient(135deg, #612a4f 0%, #8B7082 100%)' : 'rgba(255, 255, 255, 0.6)',
                ...
                >
                  ...
                  {col.id === card.columnId && <span className="text-xs opacity-80">Current</span>}
                </button>
              ))}
            </div>
          )}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'rgba(97, 42, 79, 0.4)', backdropFilter: 'blur(4px)' }}>
          <div className="w-full max-w-sm rounded-3xl p-6" style={{ background: 'white', boxShadow: '0 25px 60px rgba(97, 42, 79, 0.25)' }}>
            <h3 className="text-xl font-semibold mb-2" style={{ color: '#1a1523', fontFamily: "'Playfair Display', serif" }}>Delete this idea?</h3>
            <p className="text-sm mb-6" style={{ color: '#6b6478' }}>This action cannot be undone.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-3.5 rounded-xl font-medium transition-all" style={{ background: 'rgba(139, 112, 130, 0.1)', color: '#612a4f' }}>Cancel</button>
              <button
                onClick={onDelete}
