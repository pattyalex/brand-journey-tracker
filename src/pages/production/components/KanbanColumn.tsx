...
const KanbanColumnComponent: React.FC<KanbanColumnProps> = ({
  ...
  return (
    <div
      key={column.id}
      className={`flex-shrink-0 w-[340px] ${isTourActive && (column.id === 'shape-ideas' || tourStepIndex === 2 || (tourStepIndex === 3 && column.id === 'shape-ideas')) ? 'overflow-visible' : ''}`}
      ...
      <div
        ...
        style={{
          ...
          ...(isTourActive ? {
            ...
            } : tourStepIndex === 4 && column.id === 'ready-to-post' ? {
              border: '2px solid rgba(97, 42, 79, 0.35)',
            } : tourStepIndex === 3 ? {
              border: 'none',
            } : {}),
            ...
            <div data-kanban-scroll className={`flex-1 min-h-0 ${isTourActive && (column.id === 'shape-ideas' || tourStepIndex === 2 || (tourStepIndex === 3 && column.id === 'shape-ideas')) ? 'overflow-visible' : 'overflow-y-auto'} px-3 pt-3 pb-3 space-y-3 hide-scrollbar hover:hide-scrollbar relative rounded-[16px]`} style={{ border: hasCards ? '1.5px solid rgba(180, 168, 175, 0.2)' : '1.5px dashed rgba(180, 168, 175, 0.25)', backgroundColor: hasCards ? 'rgba(255, 252, 250, 0.7)' : 'rgba(255, 255, 255, 0.1)' }}>
              ...
              <AnimatePresence>
                {(() => {
                  ...
                  // Anatomy inline step — show anatomy card on shape-ideas, faded cards elsewhere
                  ...
                  if (isTourActive && (tourStepIndex === 1 || tourStepIndex === 4)) {
                    const cards =
                      column.id === 'ideate' ? TOUR_DEMO_CARDS :
                    ...
                    // On step 4, show Schedule on Calendar button in ready-to-post column
                    if (tourStepIndex === 4 && column.id === 'ready-to-post') {
                      return (
                        <>
                          <div style={{ opacity: 0.55 }}>
                            {cards.map((card, i) => (
