    ...
    >
      <div className="flex items-center gap-3 mb-6">
        <div>
          ...
          <p className="text-xs text-[#8B7082]">Quick answers to common questions</p>
        </div>
      </div>
      <div className="space-y-3">
        {faqs.map((faq, index) => (
          ...
          >
            ...
            >
              <span className={`text-sm font-medium ${expandedFaq === index ? 'text-[#612a4f]' : 'text-[#2d2a26]'}`}>{faq.question}</span>
              <ChevronDown className={`w-4 h-4 transition-transform ${expandedFaq === index ? 'rotate-180 text-[#612a4f]' : 'text-[#8B7082]'}`} />
            </button>
            {expandedFaq === index && (
              <div className="px-4 pb-4">
                <p className="text-sm text-[#8B7082] leading-relaxed">{faq.answer}</p>
              </div>
            )}
          </div>
        ))}
